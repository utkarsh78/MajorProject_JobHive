...
router.put("/jobs/:id", jwtAuth, (req, res) => {
  ...
  if (user.type != "recruiter") {
    // 401- unauthorised
    logger.info("Unauthorized update by applicant");
    res.status(401).json({
  ...
  Job.findOne({
    ...
    .then((job) => {
      ...
      job
        .save()
        .then(() => {
          logger.info('job update by recruiter')
          res.json({
...
router.delete("/jobs/:id", jwtAuth, (req, res) => {
  ...
  Job.findOneAndDelete({
    ...
    .then((job) => {
      ...
      logger.info("job deleted by recruiter");
      res.json({
    ...
    Recruiter.findOne({ userId: user._id })
      .then((recruiter) => {
        ...
        recruiter
          .save()
          .then(() => {
            logger.info("Recruiter has updated its personal information");
            res.json({
    ...
    JobApplicant.findOne({ userId: user._id })
      .then((jobApplicant) => {
        ...
        jobApplicant
          .save()
          .then(() => {
            logger.info("Applicant has updated its personal information");
            res.json({
              message: "User information updated successfully",
            });
          })
          ...
          Application.countDocuments({
            ...
            .then((activeApplicationCount) => {
              if (activeApplicationCount < job.maxApplicants) {
                ...
                Application.countDocuments({
                  ...
                  .then((myActiveApplicationCount) => {
                    if (myActiveApplicationCount < 10) {
                      Application.countDocuments({
                        ...
                        // {
                          ...
                          application
                            .save()
                            .then(() => {
                              logger.info("Applicant applied for a job");
                              res.json({
                      ...
                      logger.info("Applicant has already applied for 10 jobs, Hence permission denied");
                      res.status(400).json({
                        message:
                ...
                logger.info("Application Limit Reached");
                res.status(400).json({
